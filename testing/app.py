...
def days_left_color(days: int) -> str :
    '''
    This function returns a rgb color based on number of days left!!

    remaining days < 3 --> #ff0000
    3 =< remaining days =< 4 --> #00ffff
    remaining days > 4 --> #000000
    '''
...
def index():
    ...
    days_left = (datetime(2025, int(str(week_data["Foundation"][str(current_week)]['deadline'])[2:4]), int(str(week_data["Foundation"][str(current_week)]['deadline'])[0:2]), 23, 59) - datetime.now()).days
    ...
    left_targets.sort(key= lambda x: x[1])
    ...
    if not completion_status:
        ...
        for sub, remain in left_targets:
            ...
            if remain < each_sub_target(len(left_targets))[0]:
                ...
                for i in range(each_sub_target(len(left_targets) - sub_less_than_est)[0]):
                    todo.append(sub)
    ...
    return render_template('index.html',
                           ...
                           danger_color = days_left_color(days_left))
...
def analytics():
    # '''subs: is a list of tuples (sub name, level, graph img)'''
    sub = ['Maths1', 'Stats 1', 'English 1']
    return render_template('analytics.html', subs = sub)
